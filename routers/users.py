...
bycypt_context = CryptContext(schemes=['bcrypt'], deprecated='auto')
...
async def change_password(user: user_dependency,
  ...
  if not bycypt_context.verify(user_verification.password, user_model.hashed_password):
    raise HTTPException(status_code=401, detail='Error on password change')
  user_model.hashed_password = bycypt_context.hash(user_verification.new_password)
